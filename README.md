...
**Twenkid's update:** refactoring of display.py in order to use the current Pangolin's function calls for SetBounds, glDrawPoints, glDrawAxis. The last one is not exactly what the replaced DrawCameras does and the output is not like in the example. Find why etc.

https://www.youtube.com/watch?v=AFeCiMubLtM
